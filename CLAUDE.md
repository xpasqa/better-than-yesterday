...
work. Read it before brainstorming, before creating issues, and before
touching the project board.** It is binding, not advisory.
...
Non-negotiables (full gates and rationale live in the policy doc):

1. Nothing reaches the board without passing every stage, in order.
2. One feature = one card. It starts as a plain issue and becomes an
   `[EPIC]` on entering **Ready** — an epic's detailed issues are tracked in
   its checklist, never as separate cards.
3. Spec without plan means **Backlog**, not Ready.
4. One feature in **Ongoing** at a time.
5. **Done** requires verification actually run. Unrun verification keeps the
   card in **Review**, however green the tests are.

...
export async function runAgent(opts: RunAgentOptions): Promise<void> {
  ...
  while (steps < MAX_STEPS) {
    ...
    let stream: Awaited<ReturnType<typeof client.chat.completions.stream>>
    try {
    ...
    const assistantMessage = finalMessage.choices[0]?.message
    ...
    // Accumulate text for final assistant message

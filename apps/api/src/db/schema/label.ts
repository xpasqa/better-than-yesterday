...
export const label = pgTable(
  ...
  (table) => [
    unique('label_user_name').on(table.userId, table.name),
    index('label_user_seq').on(table.userId, table.seq),
    check('label_name_shape', sql`length(trim(${table.name})) between 1 and 60 and ${table.name} !~ '\\s'`),
  ],
)
